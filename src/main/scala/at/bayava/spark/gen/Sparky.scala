...
object Sparky {
	...
	def createRddWithContext(inputFileLocation: String): RDD[(LongWritable, Text)] = {
		val sc: SparkContext = setupContext

		val logData = sc.newAPIHadoopFile(inputFileLocation, classOf[MyInputFormat], classOf[LongWritable], classOf[Text])
		logData
	}

	def createRdd(inputFileLocation: String, sc: SparkContext): RDD[(LongWritable, Text)] = {
		val logData = sc.newAPIHadoopFile(inputFileLocation, classOf[MyInputFormat], classOf[LongWritable], classOf[Text])
	logData
	}
...
//			.set("spark.storage.memoryFraction", "0.1")
	...
	def main(args: Array[String]) {
		val logData = createRddWithContext("")
